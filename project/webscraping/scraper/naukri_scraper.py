"""web scraping program for naukri.com"""
import os
import time
import datetime
import pandas as pd
...
# to check if directory existed or not ?
...
def naukri_job_scraper(job_role, new_url, page):
    ...
    try:
        ...
        # find the page count from total number of jobs
            total_job = soup.find('span', class_="styles_count-string__DlPaZ").text
            count = total_job.split(" ")
            # print(total_job)
            # print(count[-2])
            click = int(count[-2])
            if click >= 20:
    ...
    for page in pages:
        ...
        for job_elem in job_elems:
            ...
            dff.to_excel(os.path.join(directory_path, f"{job_role}_NaukriJobListing_" + str(datetime.date.today()) + ".xlsx"), index=False)
